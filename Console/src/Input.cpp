...
using _input_action_t = void(const InputOptions& opt, InputContext& ctx);
...
static void ResetInput(SHORT beginX, int maxLen, int& pos, int& length, char* input, char* history)
{
    ...
    Clear(beginX, beginX + length);
    InsertText("%.*s", maxLen, history);
    length = std::min(maxLen, static_cast<int>(strlen(history)));
    pos = length;
...
static void _reset_input(const InputOptions& opt, InputContext& ctx)
{
    ...
    ctx.length = std::min(opt.maxLen, static_cast<int>(strlen(*ctx.history)));
    ctx.pos = ctx.length;
