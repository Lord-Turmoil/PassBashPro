...
static int _load_profile()
{
    ...
    int size = static_cast<int>(dirs.size());
    if (size != names.size())
        return 2;
    ...
    for (int i = 0; i < size; i++)
        pool->Add(std::make_shared<Profile>(names[i], dirs[i]));
...
static int _load_cache()
{
    ...
    // here, fp is open
    fscanf_s(fp, "%s", _cached_user, static_cast<unsigned int>(_countof(_cached_user)));
